...
# status = tk.StringVar()   # 创建字符串变量
# status.set("等待中..")   # 初始化字符串变量

# def update_label():
#     status.set("完成啦！")   # 利用变量的set方法更新内容

# def run():
#     global dirpath
#     get_file_name(dirpath)
#     special_list()
#     do_yuwen()
#     do_shuxue()
#     do_yingyu()
...
# # 选择目录
# tk.Label(root, text='教研组长表格放在：').grid(
#     row=1, column=0, padx=5, pady=5)  # 创建label 提示这是选择目录
# tk.Entry(root, textvariable=dirpath, width=50).grid(
#     row=1, column=1, padx=5, pady=5)  # 创建Entry，显示选择的目录
# tk.Button(root, text='选择目录', command=openDir).grid(
#     row=1, column=2, padx=5, pady=5)  # 创建一个Button，点击弹出打开目录窗口
# buttonRun = tkinter.Button(root, text='开始计算啦啦啦', width=15, command=run).grid(
#     row=2, column=1, padx=5, pady=5)
# tk.Label(root,textvariable=status).grid(
